...
1.MySQL에서 char와 varchar 괄호 안의 수는 **문자의 수**를 의미한다. byte 수를 의미하는 것이 아니다.

2.char(10)과 varchar(10)은 모두 문자의 수 **1 ~ 10개**의 문자열을 저장할 수 있다.  
char(10)은 문자의 수가 10개인 문자열만 저장할 수 있는 것이 아니다. 

3.두 자료형 모두 문자열의 개수가 10개를 초과하면 오류가 발생한다.
...
이처럼 **`스터디 지원자`** 와 **`스터디`** 는 각각 여러 상태가 존재한다. 또한 스터디 상태는 방장이 마음대로 바꿀 수 있는 요구사항이 있다.  **상태에 대한 update query가 빈번할 것으로 예측되고 문자열의 길이 또한 비슷** 하다.


즉 **`스터디 상태`** 와 **`지원자 상태`** 에 대하여, 현재 비즈니스 요구사항에 만족하는 자료형은 **`char`** 가 적합할 것이다.
